 ...
 * [Visual Studio Community 2015 Update 3](https://www.visualstudio.com/en-us/news/releasenotes/vs2015-update3-vs)
   * Put `is_clang = false` in your gn args to select Visual Studio.  Visual Studio is recommended until the [clang compile error with dEQP](http://crbug.com/727437) is fixed.
   ...
   * Note: Chrome is in the process of upgrading to Visual Studio 2017. ANGLE will switch over once Chrome does.
...
GN will generate ninja files by default.  Ninja can be used to compile on all platforms with one of the following commands.
```
ninja -C out/Debug
ninja -C out/Release
```
Ninja automatically calls GN to regenerate the build files on any configuration change.
Put `depot_tools` in your path as it provides ninja.

### Building with Visual Studio

Invoke GN as follows to generate Visual Studio project files:
```
gn gen out/Debug --ide=vs
```
Next run `scripts/msvs_projects.py` to generate a Visual Studio solution in `out/sln/ANGLE.sln`.
This script consolidates all GN targets in `out` into a single meta-solution.

Follow these steps in Visual Studio:
 1. Open the ANGLE solution file `out/sln/ANGLE.sln`.
 2. Select Build -> Configuration Manager
 3. In the "Active solution configuration:" drop down, select the desired configuration (eg. Release), and close the Configuration Manager.  For GN compatibility reasons each configuration is listed as `x64`, regardless of CPU architecture.
 4. Select Build -> Build Solution.
Once the build completes, the output directory for your selected configuration (eg. `Release_x64`, located next to the solution file) will contain the required libraries and dlls to build and run an OpenGL ES 2.0 application.
