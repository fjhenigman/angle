...
vk::Error WindowSurfaceVk::initializeImpl()
{
    ...
    for (auto swapchainImage : swapchainImages)
    {
        ...
        SwapchainImage swapImage(std::move(image), std::move(imageView));
        mSwapchainImages.push_back(std::move(swapImage));
    }

    ANGLE_TRY(commandBuffer->end());
    ANGLE_TRY(mRenderer->queueAndFinishCommandBuffer(*commandBuffer, 2000ull));
...
// SwapchainImage implementation.
WindowSurfaceVk::SwapchainImage::SwapchainImage()
{
}

WindowSurfaceVk::SwapchainImage::SwapchainImage(vk::Image &&imageIn, vk::ImageView &&viewIn)
    : image(std::move(imageIn)), view(std::move(viewIn))
{
}
